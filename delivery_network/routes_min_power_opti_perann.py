...
import numpy as np
from graph_perann import Graph, graph_from_file, kruskal, jean_neymar
...
for trajet in liste_trajets:
    start = perf_counter()
    print(int(trajet[0]),int(trajet[1]))
    liste_min_power.append(jean_neymar(g,int(trajet[0]),int(trajet[1])))
    stop = perf_counter()
